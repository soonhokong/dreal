...
bool nra_solver::assertLit (Enode * e, bool reason) {
    ...
    if (e->isDeduced() && e->getPolarity() == e->getDeduced() && e->getDedIndex() == id) {
        DREAL_LOG_DEBUG << "nra_solver::assertLit: " << e << " is deduced";
        return true;
    }
    _stack.push_back(e);
    rp_constraint c;
    ...
    if (rp_constraint_unfeasible(c, rp_problem_box(_rp_problem))) {
        get_explanation();
        DREAL_LOG_DEBUG << "nra_solver::assertLit: unfeasibility detected:" << e;
        return false;
    }
...
void nra_solver::pushBacktrackPoint () {
    DREAL_LOG_DEBUG << "nra_solver::pushBacktrackPoint " << _stack.size();
    // _boxes
    rp_box b;
...
void nra_solver::popBacktrackPoint () {
    DREAL_LOG_DEBUG << "nra_solver::popBacktrackPoint\t _stack.size()      = " << _stack.size();
    assert(explanation.empty());
    assert(deductions.empty());
